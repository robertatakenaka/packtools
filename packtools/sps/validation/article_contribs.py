...
class ContribValidation:
    ...
    def validate_orcid_is_registered(self):
        ...
        yield build_response(
            ...
            advice=f'Unable to automatically check {orcid} belongs to {self.contrib_name}. Check ORCID <contrib-id contrib-id-type="orcid">{orcid}</contrib-id> belongs to {self.contrib_name}',
            data=result,
            error_level=error_level,
        )
...
class ContribRoleValidation:
    ...
    @property
    def xml(self):
        parent_id = self.contrib.get("parent_id") or self.contrib.get("parent")
        return f'{self.contrib.get("contrib_full_name")} ({parent_id})'
    ...
    def validate_credit(self):
        ...
        if not valid_uri:
            if expected_uri and uri:
                advice = f'For {self.xml}, replace <role content-type="{uri}"> by <role content-type="{expected_uri}">'
            elif expected_uri:
                advice = f'For {self.xml}, replace <role>{text}</role> by <role content-type="{expected_uri}">{text}</role>'
            elif uri:
                expected_uris = list(credit_taxonomy_by_uri.keys())
                advice = f'For {self.xml}, check if <role content-type="{uri}">{text}</role> has corresponding CRediT URI: {expected_uris}'                
            elif text:
                expected_uris = list(credit_taxonomy_by_uri.keys())
                advice = f'For {self.xml}, check if <role>{text}</role> has corresponding CRediT URI: {expected_uris}'                
        yield build_response(
        ...
        if not valid_term:
            ...
            if expected_term and text:
                advice = f'For {self.xml}, replace <role{content_type}>{text}</role> by <role{content_type}>{expected_term}</role>'
            elif expected_term:
                advice = f'For {self.xml}, replace <role{content_type}></role> by <role{content_type}>{expected_term}</role>'
            elif text:
                expected_terms = self.params["credit_taxonomy_by_terms"]
                advice = f'For {self.xml}, check if <role{content_type}>{text}</role> has corresponding CRediT term: {expected_terms}'  
            else:
                expected_terms = self.params["credit_taxonomy_by_terms"]
                advice = f'For {self.xml}, check if <role{content_type}>{text}</role> has corresponding CRediT term: {expected_terms}'  
    ...
    def validate_role_specific_use(self):
        ...
        if specific_use:
            advice = f'For {self.xml}, replace {specific_use} in <role specific-use="{specific_use}"> with {expected}'
        else:
            advice = f'For {self.xml}, add contributor role type <contrib><role specific-use=""></role></contrib> with {expected}'
        yield build_response(
