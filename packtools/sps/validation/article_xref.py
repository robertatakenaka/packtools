...
class ArticleXrefValidation:
    def __init__(self, xml_tree, params=None):
        ...
        self.xrefs_by_rid = self.article_xref.xrefs_by_rid()
    ...
    def validate_element_id_has_corresponding_xref_rid(self):
        ...
        default_error_level = self.params["element_id_error_level"]
        ...
        for id, elems in self.article_xref.elems_by_id("*").items():
            for elem_data in elems:
                ...
                if label:
                    advice = (
                        f'Found <{tag} id="{id}"{ref_type_attr}>, but no corresponding <xref rid="{id}" ref-type="{ref_type}"> found. '
    ...
    def validate_attrib_name_and_value_has_corresponding_xref(self):
        ...
        dict
            A dictionary containing validation results with standard keys.
        """
        attribs = self.params["attrib_name_and_value_requires_xref"]
        error_level = self.params["attrib_name_and_value_requires_xref_error_level"]

        for attrib in attribs:
            attr_name = attrib["name"]
            attr_value = attrib["value"]
            for elem_data in self.article_xref.get_elements_selected_by_attribs([attrib]):
                tag = elem_data.get("tag")
                ...
                rid = elem_data.get("id")
                advice = (
                    f'Found {xml}, but no corresponding <xref ref-type="{ref_type}" rid="{rid}"> found. '
                    f'Mark the crossreference to {xml} using <xref ref-type="{ref_type}" rid="{rid}">'
                )
                
                yield build_response(
                    title=f'<sec {attr_name}="{attr_value}"> is linked to <xref>',
                    parent=elem_data,
                    item="sec",
                    sub_item="@id",
                    validation_type="match",
                    is_valid=is_valid,
                    expected=f'<xref rid="{id}">',
                    obtained=xrefs,
                    advice=advice,
                    data={
                        "element": elem_data,
